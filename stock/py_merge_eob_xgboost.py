...
# ======================== 读取文件 ========================
file1_path = "EOB_Buying_Method.csv"  # 选股数据
...
df_eob = pd.read_csv(file1_path, dtype={'股票代码': str})[['股票代码']]  # 只保留股票代码
...
df_eob.rename(columns={'股票代码': 'Stock Code'}, inplace=True)

# ======================== 过滤符合 EOB 选股标准的股票 ========================
filtered_stocks = df_eob.copy()
print(f"📊 选出的股票数（EOB 筛选）：{len(filtered_stocks)}")
...
# ======================== 保存 CSV 结果 ========================
output_path = "EOB_status_xgboost_lstm_transformer.csv"
