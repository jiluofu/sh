...
def get_limit_up_price(prev_close, code):
    return round(prev_close * 1.2, 2) if code.startswith(('30', '68', '300')) else round(prev_close * 1.1, 2)
...
def match_conditions(df, code):
    ...
    for i in range(1, len(df) - 1):
        ...
        is_bullish = today['收盘'] > today['开盘']
        vol_ratio = today['成交量'] / yesterday['成交量']
        upper_shadow = today['最高'] > max(today['开盘'], today['收盘'])
        limit_up_price = get_limit_up_price(yesterday['收盘'], code)
        is_limit_up = abs(today['收盘'] - limit_up_price) < 0.01
        close_higher = today['收盘'] > yesterday['收盘']

        if ENABLE_MA_ALIGNMENT and pd.notna(today['ma30']):
...
def main():
    ...
    # 读取股票信息表（包含名称和行业）
    stock_info_df = pd.read_csv(stock_info_path, dtype=str)
    stock_info_df = stock_info_df.rename(columns={'code': '股票代码', 'name': '股票名称', 'industry': '行业'})
    ...
    for code in stock_codes:
        ...
        if hit_info_list:
            for d, next_day_gain in hit_info_list:
                ...
                stock_name = stock_info_df.loc[stock_info_df['股票代码'] == code, '股票名称'].values[0] \
                    if code in stock_info_df['股票代码'].values else ""
                industry = stock_info_df.loc[stock_info_df['股票代码'] == code, '行业名称'].values[0] \
                    if code in stock_info_df['股票代码'].values else ""

                results.append([
                    code, stock_name, industry, d, f"{next_day_gain:.2f}%", sell_date, buy_price,
                    sell_price, gain_pct_str, chip_status, chip_concentration
                ])

                print(
                    f"✅ 命中: {code} - {stock_name} - {d}｜次日涨幅: {next_day_gain:.2f}%｜"
    ...
    if results:
        result_df = pd.DataFrame(results, columns=[
            "股票代码", "股票名称", "行业", "命中日期", "次日涨幅", "建议卖出日期",
            "买入价", "卖出价", "涨幅 (%)", "筹码状态", "集中度估算"
        ])
