...
end_date = pd.bdate_range(end=today, periods=TRADING_DAYS_AGO+1)[0].strftime("%Y%m%d")  # 向前推 N 个交易日
# 配置开始和结束日期
START_DATE = "20240101"
END_DATE = end_date
print(f"📅 END_DATE 自动调整为: {END_DATE}（{TRADING_DAYS_AGO} 个交易日前）")


NUM_DAYS = 5

# 将 END_DATE 转换为 datetime 格式
end_date_dt = datetime.strptime(END_DATE, "%Y%m%d")
# 计算 END_DATE + NUM_DAYS 个交易日后的日期
future_date_adjusted = (end_date_dt + pd.tseries.offsets.BDay(NUM_DAYS)).strftime("%Y%m%d")

print(f"📅 未来 {NUM_DAYS} 个交易日后的日期: {future_date_adjusted}")
...
def process_stock_data(stock_code, df, num_simulations=10000, num_days=NUM_DAYS):
    ...
    # 修改：获取 `TRADING_DAYS_AGO` 个交易日前的收盘价
    trading_days_ago_price = df.iloc[-(TRADING_DAYS_AGO + 1), 0] if len(df) > TRADING_DAYS_AGO else None

    # 计算预测价格相对于 `TRADING_DAYS_AGO` 个交易日前收盘价的涨幅
    increase_percent = round(((price_mean - trading_days_ago_price) / trading_days_ago_price) * 100, 2) if trading_days_ago_price else None
    
    print(f"{stock_code} 未来 {num_days} 天股价预测区间：")
    print(f" - 95% 置信区间: [{price_5_percentile}, {price_95_percentile}]")
    print(f" - 预测均值: {price_mean}")
    print(f" - 昨日收盘价: {trading_days_ago_price}")
    print(f" - 预测比昨日收盘价变化百分比: {increase_percent}%")
    
    return [stock_code, price_mean, trading_days_ago_price, increase_percent]

def analyze_stock_phase(stock_code, df, predicted_price, increase_percent):

    if increase_percent == None or increase_percent <= 0:
        return None
...
print("✅ 预测比昨日收盘价变化大于 1% 的股票代码已保存到 filtered_stocks.txt，格式为双引号包围，逗号分隔")
