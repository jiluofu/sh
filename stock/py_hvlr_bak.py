...
def estimate_chip_concentration(df, date, window=20):
    ...
    if mean == 0:
        return "异常", None
    concentration = round(100 * (1 - std / mean), 2)  # 粗略集中度
    if concentration > 85:
        status = "高度集中"
...
def main():
    ...
    for code in stock_codes:
        ...
        if hit_dates:
            for d in hit_dates:
                ...
                # if chip_concentration >= 96 and chip_concentration <= 98:
                if chip_concentration <= 100:    
                    results.append([
                        code, d, sell_date, buy_price, sell_price, gain_pct_str, chip_status, chip_concentration
                    ])
                    print(
                        f"✅ 命中: {code} - {d}｜卖出: {sell_date if sell_date else '未触发'}｜涨幅: {gain_pct_str}｜"
    ...
    if results:
        result_df = pd.DataFrame(results, columns=[
            "股票代码", "命中日期", "建议卖出日期", "买入价", "卖出价", "涨幅 (%)", "筹码状态", "集中度估算"
        ])
